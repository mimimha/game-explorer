...
def _extract_intent(prompt):
    ...
    system = (
        ...
        '예: "동물 나오는 게임"→["동물"]), '
        'subject_terms(배열, 사용자가 언급한 핵심 소재·요소를 한국어+영어 둘 다 — '
        '생물·사물·장소·테마 등 게임에 그것이 나오길 바라는 구체 명사는 모두 넣는다. '
        '장르/추상으로 해석하지 말고 그 단어 자체를 넣어라. '
        '예: "토끼"→["토끼","rabbit"], "농장"→["농장","farm"], "좀비"→["좀비","zombie"], '
        '"우주"→["우주","space"], "마법"→["마법","magic"], "고양이"→["고양이","cat"]. '
        '사전에 없어도 됨. "재미","게임" 같은 일반/추상어만 제외), '
